...
goal_position = [0; 0.286; 0.2045];

goal = [goal_rotation, goal_position];
goal = [goal; 0, 0, 0, 1];

base_rotation = rot_z(0) * rot_y(0) * rot_x(0);
base_position = [0.286; 0; 0.2045];
base = [base_rotation, base_position];
base = [base; 0, 0, 0, 1];

goal = base \ goal;

theta3_list = joint3(p_04', p_02', axis3', goal);
...
for theta3 = theta3_list
    %[
     %   cos(theta3) 0 sin(theta3) l_2 - sqrt(l_1^2+l_2^2) * sin(90 - theta3 - atan2(l_1, l_2));
      %  0 1 0 0;
       % -sin(theta3) 0 cos(theta3) l_1+sqrt(l_1^2+l_2^2) * cos(90 - theta3 - atan2(l_1, l_2));
        %0 0 0 1
        %]
    ...
    [theta1_list, theta2_list] = joint1joint2(p_04', e_3, goal, axis1', axis2');
